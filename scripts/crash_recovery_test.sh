...
#   Scenario 5 (freeze, plan 090): commit+CHECKPOINT edges 0->1, 0->2, then a COMMITTED
#                             gph_freeze(horizon) whose GenericXLog page rewrites live ONLY in the
#                             WAL, crash, restart -> assert the frozen state was REDONE (visibility
#                             unchanged, relfrozenxid advanced, metapage no-op at the same horizon,
#                             0 unfrozen records at a higher one) — the durability half of
#                             test/graph_freeze_test.sql. Stock mirror: scripts/pg17_crash_recovery_test.sh.
#
