...
SQL
  ...
  # `SELECT txid_current()` is NOT a sufficient barrier (measured here: a commit whose only
  # WAL is its own commit record still takes the async path). A committed txn that writes
  # DATA WAL does force a synchronous XLogFlush through its commit record, which follows
  # every freeze record; the freeze pages themselves stay WAL-only, so the restart still
  # proves GenericXLog REDO.
  $PSQL -c "INSERT INTO graph_store.entities VALUES (9000, '"'"'wal-flush barrier'"'"', '"'"'[9000,0,0,0,0,0,0,0]'"'"'::vector);" >/dev/null
