#!/usr/bin/env bash
#
# join_order_test.sh — build + test the cross-modal join-order heuristic (FR-6 / DEV-1170).
#
# Builds the join_order extension (src/planner/) against the MSVBASE fork image via PGXS, then
# runs test/join_order_test.sql, which asserts the C port makes BIT-IDENTICAL decisions to the
...
# Usage: scripts/join_order_test.sh [image]
set -euo pipefail
...
docker run --rm --entrypoint bash \
  ...
  echo "=== FR-6 join-order parity suite (DEV-1170) ==="
  $B/psql -p 5432 -d postgres -v ON_ERROR_STOP=1 -f /tmp/join_order_test.sql
  rc=$?
  $B/pg_ctl -D $D -m fast stop >/dev/null 2>&1 || true
  exit $rc
' 2>&1 | grep -vE "redirecting log|logging collector"
echo "[join_order_test] PASS — C port matches the join_order_ref.py reference (FR-6)."
