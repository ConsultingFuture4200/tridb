...
container (docker exec -i <container> psql -f -, the bench/wiki_h2h.py convention) that:
  ...
  (d) builds the HNSW index exactly as tools/wiki_engine_load does
      (USING hnsw(embedding) WITH (dimension=D, distmethod=l2_distance)) and runs a
      top-k health probe (a loaded row's own vector must return k rows);
...
def vec_literal(v: Iterable[float]) -> str:
    # Postgres float8[] literal (matches tools/wiki_engine_load._vec_literal).
...
# ======================================================================================
def sql_prologue(table: str, dim: int, force: bool) -> str:
    L = [
...
def entity_copy_row(dense_id: int, qid: int, p31: list[int], vec: list[float]) -> str:
    return f"{dense_id}\t{qid}\t{int_array_literal(p31)}\t{vec_literal(vec)}\n"
...
def sql_hnsw_and_health(table: str, dim: int, probe_vec: list[float], k: int) -> str:
    """HNSW build exactly as tools/wiki_engine_load._build_load_sql, plus a health
    probe: top-k on a loaded row's own vector must return k rows via the index."""
    return f"""\\echo #WDL HNSW_BUILD_START
CREATE INDEX {table}_hnsw ON {table} USING hnsw(embedding)
    WITH (dimension = {dim}, distmethod = l2_distance);
...
BEGIN
  SELECT count(*) INTO got FROM (
    SELECT id FROM {table} ORDER BY embedding <-> '{vec_literal(probe_vec)}' LIMIT {k}
  ) q;
  IF got <> {k} THEN
...
def iter_load_sql(
    ...
    *,
    ...
    yield sql_hnsw_and_health(table, dim, probe_vec, min(PROBE_K, n))
