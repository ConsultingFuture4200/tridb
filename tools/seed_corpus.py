...
def main():
    ...
    for i in range(args.entities):
        ...
        entities.append(
            {"id": i, "timestamp": timestamp, "chunk": chunk, "embedding": embedding}
        )
    ...
    edges = []
    for i in range(args.entities):
        k = min(args.edges_per_node, args.entities - 1)
        dsts = rng.choice(args.entities, k, replace=False)
    ...
    for qid in range(10):
        ...
        start_time = rng.integers(args.time_min, args.time_max - 28)
    ...
    # Generate load.sql — paths are absolute so psql can be invoked from any directory
    entities_path = os.path.abspath(os.path.join(args.out, "entities.csv"))
    edges_path = os.path.abspath(os.path.join(args.out, "edges.csv"))
    with open(os.path.join(args.out, "load.sql"), "w") as f:
        f.write(
            "CREATE TABLE entity(id int primary key, timestamp int, chunk text, embedding float8[]);\n"
        )
        f.write("CREATE TABLE related_to(src int, dst int);\n")
        f.write(
            f"\\copy entity(id,timestamp,chunk,embedding) FROM '{entities_path}' CSV HEADER;\n"
        )
        f.write(f"\\copy related_to(src,dst) FROM '{edges_path}' CSV HEADER;\n")

    print(
        f"Wrote {args.entities} entities, {len(edges)} edges, 10 queries to {args.out}"
    )
