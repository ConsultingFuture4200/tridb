...
def build(args) -> tuple[str, dict]:
    ...
    # ----------------------------------------------------------------- SQL --
    entities = [(j, int(ts[j]), emb[j]) for j in range(n)]
    sql = build_sql(
        manifest=manifest,
        entities=entities,
        edges=edges,
        source="tools/bench_corpus.py",
    )
    return sql, manifest
...
def build_sql(
    *,
    ...
    entities,
    edges,
    source: str = "tools/bench_corpus.py",
) -> str:
    """Emit the canonical LIVE-benchmark `#BENCH` SQL for a corpus.

    THIS IS THE SINGLE SOURCE OF TRUTH for the #BENCH SQL format. Both the
    synthetic path (:func:`build`) and the real-dataset path
    (``tools/real_corpus.py``) call it, so the format the downstream consumer
    (scripts/bench_live.sh -> bench/live_report.py) sees CANNOT drift between
    them. The only inputs are the manifest's public metadata plus the explicit
    entity rows and edges — no RNG, so a real-dataset corpus (whose vectors are
    loaded from disk, not generated from a seed) emits identical SQL.

    Parameters
    ----------
    manifest : dict
        The public manifest (needs ``dim``, ``hubs``, ``fanout``, ``k``,
        ``queries``; each query is ``{"qid", "src", "embedding", "window"}``).
    entities : iterable of (id, ts:int, embedding)
        Every entity row to insert, in id order.
    edges : iterable of (src, dst)
        The native adjacency edges (hub -> dst).
    source : str
        The generator name stamped into the file header comment.
    """
    n = manifest["entities"]
    dim = manifest["dim"]
    k = manifest["k"]
    ...
    for i in range(0, len(entities), batch):
        vals = []
        for eid, ets, eemb in entities[i : i + batch]:
            vals.append(
                f"({eid},'chunk {eid}',{int(ets)},'{_vec_literal(eemb)}'::float8[])"
            )
        w(
