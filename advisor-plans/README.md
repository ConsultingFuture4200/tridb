...
| 077 | Replace full-reach stock TJS materialization with a bounded pull graph leg | P1 | L | HIGH | 071, 072, 074, 075 | design gate + stock PG16/17 | **GATE-OPEN** (Steps 1-2 done, merged `cb5742b`: ADR-0020 Proposed + `test/tjs_pg_tr1_test.sql` negative control (fails by design pre-fix, unwired; advisor re-ran: filter-first k=1 = full 1200-edge reach, 2.00× scaling). AWAITING maintainer approval of ADR-0020 (5 open decisions listed in the ADR: budget default 65536, shared seedless pool, membership-now/PPR-later, keep bfs as oracle, censor flag at Step 5) before Steps 3-5) |
