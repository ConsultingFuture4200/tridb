...
def resolve_seed(manifest: dict, flag_seed: int | None) -> int:
    """The manifest's recorded seed is authoritative for grading (plan 089).

    The corpus was generated by tools/bench_corpus.py, which writes its seed into
    the manifest; rebuilding with any other seed silently grades against a wrong
    oracle. So: use manifest["seed"] when present; a --seed flag must AGREE with
    it (conflict = hard error, never warn-and-continue). Only a legacy manifest
    without a "seed" key falls back to the flag (default 42), with a warning.
    """
    manifest_seed = manifest.get("seed")
    if manifest_seed is not None:
        if flag_seed is not None and flag_seed != manifest_seed:
            raise SystemExit(
                f"--seed {flag_seed} conflicts with the manifest's recorded seed "
                f"{manifest_seed}; grading must use the seed the corpus was "
                f"generated with (drop --seed, or fix the manifest)"
            )
        return int(manifest_seed)
    seed = flag_seed if flag_seed is not None else 42
    print(
        f"[live_report] WARNING: manifest records no seed (legacy); "
        f"grading with --seed {seed}"
    )
    return seed
...
def main(argv=None) -> int:
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="corpus seed override; default is the manifest's recorded seed "
        "(errors on conflict; legacy manifests without a seed fall back to "
        "this flag, default 42)",
    )
    ...
    report = build_report(text, manifest, resolve_seed(manifest, args.seed))
