...
-- ABORT atomicity: edges added in a rolled-back txn must NOT persist in the counter (the increment
-- is registered in the same GenericXLogState as the edge slot, so it rolls back with the page image).
BEGIN;
...
BEGIN
    IF gph_edge_count() <> 1504 THEN
        RAISE EXCEPTION 'after ROLLBACK edge_count % <> 1504 (edge-count increment did not roll back atomically)', gph_edge_count();
    END IF;
    RAISE NOTICE 'PASS abort: 2 edges visible in-txn (1506), rolled back to 1504 after ROLLBACK';
END $$;
